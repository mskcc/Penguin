...
file3="${filtered_file_base}_${suffix_ecDNA_file2}"  # Single output file with "gene_exists_in_impact" annotation
...
cut --complement -f3,4 "$formatted_file" > "$no_cn_file"

# Step 5: Filter the noCN file based on p_ecDNA >= 0.4
awk -F'\t' -v threshold="$p_ecDNA_cutoff" 'NR==1 || $3 >= threshold' "$no_cn_file" > "$filtered_file"

# Step 6: Use the filtered file for further processing (comparing it with the gene list)
awk -F'\t' 'NR==FNR {genes[$1]; next} 
  FNR==1 {print $0 "\tgene_included_in_impact"; next} 
  {print $0 "\t" (($2 in genes) ? "yes" : "no")}' "$gene_list_file" "$filtered_file" > "$file3"
...
echo "Annotated output file with gene impact information: $file3"
